  "use client";
    ...
    const Home = () => {
        ...
        useEffect(() => {
            const setupFirebase = async () => {
                try {
                    ...
                    const docRef = doc(db, 'artifacts', appId, 'users', userId, 'presets', 'default');
        ...
        // A particle object
        const Particle = useCallback((x, y) => {
            let rotation = 0;
        ...
        const animate = useCallback(() => {
            ...
            if (particles.length < particleCount) {
                for (let i = particles.length; i < particleCount; i++) {
                    particles.push(Particle(Math.random() * canvas.width, Math.random() * canvas.height));
                }
        ...
        useEffect(() => {
            ...
            const resizeCanvas = () => {
                ...
                for (let i = 0; i < particleCount; i++) {
                    window.particles.push(Particle(Math.random() * canvas.width, Math.random() * canvas.height));
                }
            };
        ...
        // Main UI rendering
        return (
            <div className={`relative min-h-screen text-gray-100 font-sans ${isDarkMode ? 'bg-zinc-950' : 'bg-gray-100'}`}>
                {/* Main Canvas Area */}
